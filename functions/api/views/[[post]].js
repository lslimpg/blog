...
async function queryDB(ctx) {
  ...
  const query = `
    ...
    WHERE post = ?1
  `;
  const total = await ctx.env.DB.prepare(query).bind(`${post}`).first("totalViews");
  return total;
}
...
async function updateDB(insert, totalViews, ctx) {
  ...
  `;
  try {
    const { success } = await ctx.env.DB.prepare(query).bind(`${post}`, totalViews, lastUpdated).run();
    return success;
  } catch (e) {
...
async function getPageViewThruCFAnalytics(ctx) {
  const params = ctx.params.post;
  const today = new Date(), yesterday = new Date(today.getTime() - (24 * 60 * 60 * 1000));
  let pathUrl = "/";
...
export async function onRequestGet(ctx) {
  try {
    ...
    if (!res) {
      return new Response(null, { status: 500, statusText: "DB update failed" });
    }
